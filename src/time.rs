...
//!
//! Ported from `unidpp-trust`'s `time` (itself ported from
//! `unidpp-registry`'s, itself ported from `unidpp-resolver`'s, itself
//! ported from `unidpp-core`'s `unidpp_model::time` — the house
//! doctrine): hand-rolled on purpose — no external time dependency,
//! exact integer arithmetic (seconds since the UNIX epoch;
//! sub-second precision is not needed at the archive service), strict
//! parsing, canonical display with the trailing `Z`. All archive
//! stamps are UTC; local time is a presentation concern.
//! [`Timestamp::to_model`] / [`Timestamp::from_model`] convert to the
//! core's nanosecond-precision [`unidpp_model::Timestamp`] at the
//! signatif boundary (second resolution; fractions are accepted on
//! input and discarded).
...
impl Timestamp {
    ...
    /// this is only the as-of default and the notarization instant
    /// (which is then journaled and preserved across replays).
        ...
        if pos < b.len() {
            match b[pos] {
                ...
                b'+' | b'-' => {
                    ...
                    offset_secs = sign * (oh * 3_600 + om * 60);
                }
                _ => return Err(err()),
            }
            if pos != b.len() {
